...
\begin{frame}
	...
	\note{要了解我们的技术路线,先看一看相关领域,密切相关的领域包括 few-shot extreme classification, openset face recoginition, fine grained classification,我们会从相关领域的文献,借鉴思路}
\end{frame}
...
\begin{frame}
	...
	\end{itemize}
	\note{对于行人再识别这一领域主流方法分为三类,
	
	第一类转化为分类问题方法缺点在于样本不丰富情况下鉴别能力不足,

	第二类转化为验证问题缺点在于计算量巨大

	我们采用直接学习嵌入特征的方法,首先简单介绍我们的基础模型
}
\end{frame}
...
\begin{frame}
	...
	\end{columns} 
	\note{只需看主干支路即可,从图片输入,特征提取,挖掘难样本,到计算损失, 特征提取采用resnet}
\end{frame}
...
\begin{frame}
	...
	\end{figure}
	\note{我们要实现的目标是,属于同一类的距离尽量接近,如图所示冷色掉表示距离小.
	也就诉说,目标是从左侧的杂乱无章到右侧的分清类别. 那么为什么要进行难样本挖掘, 因为整个距离矩阵不是所有元素都需要更新的,我们要用最快的速度获得最好的性能. 
	}
\end{columns} 
...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{equation}
	\end{itemize}
	\note{在之前的步骤中,距离矩阵每一行会选择处两个样本对,计算相对距离,从而计算损失,反向传播}
\end{frame}
...
\begin{frame}
	...
	\end{figure}
	\note{上策为基础模块,瞎扯为总体框架. 基础模块具有两个功能,1 通道级别调整权重实行注意力机制, 2 提取本阶段分辨率下的全局特征表示,总体框架中将各个阶段特征表示拼接 值得注意的是,我们几乎没有增加计算量}
\end{frame}
...
\begin{frame}
	...
	\end{figure}
	\note{全局挖掘,利用历史损失函数体攻的信息,在形成batch的时候,选取有正对性的难样本 同样我们也没有增加计算量 }
\end{frame}
...
\begin{frame}{现阶段实验}
	\begin{table}
		\begin{tabular}{lrrr}
			...
			\end{tabular}
	\end{table}
	\note{在寒假我进行了初步实验,有的效果好,有的效果一般. 期中97.59,已经超过了人类表现,据我了解目前只有两篇文章做到这一点. 从效果来说,挖掘难样本具有可行性,改改网络结构效果不明显. 但是也不仅仅是效果,需要阅读文献,提出新的想法
	行人再识别发展很快,除去传统数据集,今年提出了新的问题和新的数据集,因此,我还有很多工作要做}
\end{frame}
...
\begin{frame}{现阶段实验}
	\begin{table}
		...
		\end{tabular}
	\end{table}
	\note{行人再识别发展很快,除去传统数据集,今年提出了新的问题和新的数据集,因此,我还有很多工作要做}
\end{frame}
